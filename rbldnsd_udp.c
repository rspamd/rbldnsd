...
#endif
    if (n < 0 && errno == EINTR && interrupted++ < 4) continue;
...
#ifdef SO_RXQ_OVFL
  overflow = mmap(NULL, (size_t)fd_limit * sizeof(*overflow),
                  PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
  if (overflow == MAP_FAILED) { overflow = NULL; return -1; }
...
void rbldnsd_udp_overflow(int fd, uint32_t sample) {
  if (!overflow || fd < 0 || (unsigned)fd >= overflow_count) return;
  ...
  for (;;) {
    ...
    if (!delta || delta >= UINT32_C(0x80000000)) return;
    if (__atomic_compare_exchange_n(&overflow[fd], &previous, sample, 0,
...
void rbldnsd_udp_received(int fd, const struct msghdr *msg) {
  ...
  if (msg->msg_flags & MSG_CTRUNC) return;
  ...
  for (struct cmsghdr *c = CMSG_FIRSTHDR(&copy); c; c = CMSG_NXTHDR(&copy, c)) {
    ...
    if (c->cmsg_len < CMSG_LEN(0) || c->cmsg_len > left) break;
