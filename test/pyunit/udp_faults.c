...
#endif
void rbldnsd_control_send_error(unsigned n) {
  dropped += n;
}

void rbldnsd_control_receive_drop(unsigned n) {
  received += n;
}
...
static void reset(void) {
  ...
  calls = length = 0;
  dropped = received = offset = 0;
}

static void add(int result, int error) {
...
int main(void) {
...
#endif
  rbldnsd_udp_send(42, messages, 5);
  assert(offset == 5 && !dropped && calls == length);
  reset();
#ifdef TEST_BATCH
  add(2, 0);
#else
  ...
  add(0, 0);
  rbldnsd_udp_send(42, messages, 5);
  assert(calls == 1 && dropped == 5);
#endif
  uint32_t baseline[2] = {0};
  overflow = baseline;
  overflow_count = 2;
  received = 0;
  rbldnsd_udp_overflow(1, 10);
  rbldnsd_udp_overflow(1, 10);
  rbldnsd_udp_overflow(1, 9);
  rbldnsd_udp_overflow(1, 12);
  rbldnsd_udp_overflow(2, 100);
  assert(received == 12 && baseline[1] == 12);
  baseline[1] = UINT32_MAX - 2;
  received = 0;
  rbldnsd_udp_overflow(1, 3);
  assert(received == 6);
#ifdef SO_RXQ_OVFL
  union {
    struct cmsghdr align;
    char data[CMSG_SPACE(sizeof(uint32_t))];
  } buf;
  struct msghdr hdr = {0};
  hdr.msg_control = buf.data;
  hdr.msg_controllen = sizeof(buf.data);
  ...
  uint32_t sample = 7;
  memcpy(CMSG_DATA(c), &sample, 4);
  hdr.msg_flags = MSG_CTRUNC;
  rbldnsd_udp_received(1, &hdr);
  assert(received == 6);
  hdr.msg_flags = 0;
  c->cmsg_len = 999;
  rbldnsd_udp_received(1, &hdr);
  ...
  assert(child >= 0);
  if (!child) {
    rbldnsd_udp_overflow(1, 100);
    _exit(0);
  }
  int status;
  assert(waitpid(child, &status, 0) == child && WIFEXITED(status));
  rbldnsd_udp_overflow(1, 104);
  assert(received == 4);
  munmap(overflow, 2 * sizeof(*overflow));
  overflow = NULL;
#endif
