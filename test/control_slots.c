...
int main(int argc,char **argv) {
  ...
  int slot=rbldnsd_control_slot_alloc(1), commands[2], replies[2];
  assert(slot>=0 && pipe(commands)==0 && pipe(replies)==0);
  pid_t child=fork(); assert(child>=0);
  if(!child) {
    close(commands[1]); close(replies[0]); rbldnsd_control_child();
    assert(rbldnsd_control_worker(slot)); send_byte(replies[1]);
    byte(commands[0]);
    /* Quarantine must survive an old worker changing into draining state. */
    rbldnsd_control_draining(); rbldnsd_control_query(10,20,0);
    send_byte(replies[1]); byte(commands[0]); _exit(0);
  }
  close(commands[0]); close(replies[1]); byte(replies[0]);
  rbldnsd_control_release_generation(1);
  int other=rbldnsd_control_slot_alloc(2); assert(other>=0 && other!=slot);
  send_byte(commands[1]); byte(replies[0]);
  int third=rbldnsd_control_slot_alloc(2); assert(third>=0 && third!=slot);
  send_byte(commands[1]); int status;
  ...
  rbldnsd_control_release(slot); rbldnsd_control_release(other); rbldnsd_control_release(third);
  ...
  other=rbldnsd_control_slot_alloc(5); assert(other>=0 && other!=slot);
  send_byte(commands[1]); byte(replies[0]);
  third=rbldnsd_control_slot_alloc(5); assert(third>=0 && third!=slot);
