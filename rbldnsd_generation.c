...
static int generation_write(int fd, const void *data, size_t length) {
  ...
  while (length) {
    ssize_t transferred = write(fd, cursor, length);
...
static void generation_owner_control(struct ev_loop *loop, ev_io *w, int revents) {
  char command;
  ssize_t transferred = read(w->fd, &command, 1);
  if (!transferred || (transferred < 0 && errno != EAGAIN && errno != EINTR)) {
...
static void generation_owner(int fd) {
  ...
  while ((dataset = nextdataset(dataset)) != NULL) {
    for (struct dsfile *file = dataset->ds_dsf; file; file = file->dsf_next) {
      struct generation_stamp stamp = {file->dsf_stamp, file->dsf_size};
      if (!generation_write(fd, &stamp, sizeof(stamp))) {
        _exit(1);
      }
    }
  }
  ...
  for (struct zone *zone = zonelist; zone; zone = zone->z_next) {
    struct generation_zone state = {zone->z_stamp, zone->z_expires};
    if (!generation_write(fd, &state, sizeof(state))) {
      _exit(1);
    }
  }
  ...
  char command;
  if (read(fd, &command, 1) != 1 || command != 'A') {
    _exit(1);
  }

  for (int i = 0; i < nworkers; ++i) {
    if (!generation_write(workers[i].control, "A", 1)) {
      _exit(1);
    }
  }
  generation_activated = 1;
  if (!generation_write(fd, "A", 1)) {
    _exit(1);
  }
  ...
  for (int i = 0; i < nworkers; ++i) {
    if (workers[i].pid) {
      shutdown(workers[i].control, SHUT_WR);
    }
  }
...
static void generation_guardian(int fd) {
  ...
  struct generation_process *generations[] = {&generation_active, &generation_retiring};
  for (unsigned i = 0; i < 2; ++i) {
    if (generations[i]->id) {
      if (generations[i]->guardian) {
        close(generations[i]->fd);
      }
      close(generations[i]->life);
    }
  }
  ...
  if (!owner) {
    ...
    setpgid(0, 0);
    char begin;
    if (read(pair[1], &begin, 1) != 1 || begin != 'L') {
      _exit(1);
    }
  ...
  if (!generation_write(pair[0], "L", 1)) {
    kill(-owner, SIGKILL);
    _exit(1);
  }
  ...
  for (;;) {
    ...
    struct pollfd fds[2] = {{stopping ? -1 : fd, POLLIN, 0}, {stopping ? -1 : pair[0], POLLIN, 0}};
    int poll_result = poll(fds, 2, 50);
    ...
    if (fds[0].revents) {
      char command;
      ssize_t transferred = read(fd, &command, 1);
      if (transferred != 1 || command == 'Q') {
        stopping = 1;
        deadline = ev_time() + 6;
        shutdown(pair[0], SHUT_WR);
        kill(owner, SIGTERM);
      } else if (command == 'A') {
        if (!generation_write(pair[0], &command, 1)) {
          break;
        }
      } else if (command == 'H' || command == '1' || command == '2') {
        kill(owner, command == 'H' ? SIGHUP : command == '1' ? SIGUSR1 : SIGUSR2);
      }
    }
    if (fds[1].revents && !stopping) {
      char buffer[4096];
      ssize_t transferred = read(pair[0], buffer, sizeof(buffer));
      if (transferred <= 0 || !generation_write(fd, buffer, transferred)) {
        break;
      }
    }
  }
...
static int generation_start(struct ev_loop *loop) {
  ...
  if (generation_active.guardian && !nextdataset2reload(NULL) &&
      !call_hook(reload_check, (zonelist))) {
    return 1;
  }
...
static int generation_exited(pid_t pid) {
  struct generation_process *generations[] = {&generation_candidate, &generation_active,
                                              &generation_retiring};
  for (unsigned i = 0; i < 3; ++i) {
    struct generation_process *generation = generations[i];
    if (generation->guardian != pid) {
      continue;
    }
    if (!generation->owner && generation->message) {
      ssize_t transferred = read(generation->fd, generation->message + generation->used,
                                 generation_message_size - generation->used);
      if (transferred > 0) {
        generation->used += transferred;
      }
      if (generation->used >= sizeof(pid_t)) {
        memcpy(&generation->owner, generation->message, sizeof(pid_t));
      }
    }
    if (generation->owner) {
    ...
    rbldnsd_control_release_generation((unsigned)generation->id);
    generation->guardian = 0;

    /* Keep the generation occupied until its inherited liveness pipe closes. */
    if (i == 0) {
      rbldnsd_control_reload(0);
    }
    if (i == 0) {
      dslog(LOG_WARNING, 0, "candidate failed; retaining active generation");
    }
...
static void generation_collect(void) {
  ...
  for (unsigned i = 0; i < 3; ++i) {
    struct generation_process *generation = generations[i];
    if (!generation->id || generation->guardian) {
      continue;
    }
    char ignored;
    if (read(generation->life, &ignored, 1) != 0) {
      continue;
    }
...
static void generation_supervise(struct ev_loop *loop, ev_timer *w, int revents) {
  generation_collect();
  struct generation_process *generation = &generation_candidate;
  if (generation->guardian) {
    if (ev_time() >= generation->deadline) {
      ...
      (void)write(generation->fd, "Q", 1);
      if (generation->owner) {
        kill(-generation->owner, SIGKILL);
      }
      kill(generation->guardian, SIGKILL);
      generation->deadline = ev_time() + 7;
    }
    if (generation->state == 0) {
      ...
      if (!generation->owner && generation->used >= sizeof(pid_t)) {
        memcpy(&generation->owner, generation->message, sizeof(pid_t));
      }
      if (generation->used == generation_message_size) {
        if (write(generation->fd, "A", 1) == 1) {
          generation->state = 1;
        }
      }
    }
    if (generation->state == 1 && (!generation_active.id || generation_active.guardian)) {
      char activated;
      if (read(generation->fd, &activated, 1) == 1 && activated == 'A') {
        unsigned char *cursor = generation->message + sizeof(pid_t);
        struct dataset *dataset = NULL;
        while ((dataset = nextdataset(dataset)) != NULL) {
          for (struct dsfile *file = dataset->ds_dsf; file; file = file->dsf_next) {
            struct generation_stamp stamp;
            memcpy(&stamp, cursor, sizeof(stamp));
            cursor += sizeof(stamp);
            file->dsf_stamp = stamp.stamp;
            file->dsf_size = stamp.size;
          }
        }
        for (struct zone *zone = zonelist; zone; zone = zone->z_next) {
        ...
        rbldnsd_control_reload(1);
        memset(generation, 0, sizeof(*generation));
        if (generation_retiring.guardian) {
          (void)write(generation_retiring.fd, "Q", 1);
        }
  ...
  can_reload = !generation_candidate.id && !generation_retiring.id &&
               (!generation_active.id || generation_active.guardian);
  if (can_reload && pending_reload) {
    generation_start(loop);
  }
}

static void generation_shutdown(void) {
  struct generation_process *generations[] = {&generation_active, &generation_candidate,
                                              &generation_retiring};
  ev_tstamp deadline = ev_time() + 7;
  for (unsigned i = 0; i < 3; ++i) {
    if (generations[i]->guardian) {
      (void)write(generations[i]->fd, "Q", 1);
    }
  }
  ...
  for (unsigned i = 0; i < 3; ++i) {
    ...
    wait_worker_shutdown(generation->guardian, deadline);
    if (generation->owner) {
      kill(-generation->owner, SIGKILL);
    }
    close(generation->fd);
